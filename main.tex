...
\]
The bound is uniform in $f$ and $t$.  It controls the optimal order of growth,
but does not preserve Uhlig's sharp one-copy leading constant.
...
\]
which approach one.
Unlike a purely combinatorial batch-code guarantee, the schedule here is
computed from the input addresses and its Boolean gates are included in the
circuit bound.
\end{abstract}
...
This paper
  ...
  & $O_\gamma(2^n/n)$
  & optimal worst-case order throughout every fixed exponential range \\
\bottomrule
\end{tabularx}
\end{center}
...
group of $g$ requests, our scheduler has size
...
This scheduler cost, rather than the mere existence of disjoint recovery sets,
is what drives the equal-block induction.
...
We claim no novelty for evaluation coding, affine-line recovery, disjoint
recovery as a combinatorial ingredient, or generic sorting and multiselection
routing primitives.  The claimed contribution is the quantitative combination
of a fine-grained deterministic disjoint-line scheduler, explicit accounting
for the scatter/gather cost, and the bounded-congestion composition that makes
those costs affordable, yielding the exponential-range mass-production
theorem.  A sufficiently sharper
implementation of the deletion-based batch decoder could conceivably improve
the scheduler term; in particular, a $\widetilde O(tq)$ disjoint-recovery
scheduler would remove the present recursion, as discussed in
\cref{sec:discussion}.
...
equal-block induction.  It proves a rational, discrete-exponent form of
...
achievable at the optimal worst-case order.  The two immediate quantitative
questions are whether recursion is necessary and where the plateau ends as
the copy exponent approaches one.
