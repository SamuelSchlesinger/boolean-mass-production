...
the answers.  The circuits are nonuniform; an efficient algorithm for
constructing their fixed scheduling data remains open.  A Lean companion
verifies the Boolean upper bounds.
...
Kretschmer established quantum analogues of Uhlig's theorem
\cite{Kretschmer2023}; recent work connects mass production to coherent
data loading and quantum algorithms \cite{HugginsKhattarWiebe2025}.
Our Boolean theorem yields a matching upper and lower bound for approximate
state preparation in a finite gate set.
...
of $1/\sqrt t$.  The quantum arguments are written proofs and are outside
the Lean companion.
...
An accompanying Lean~4 development verifies the Boolean recursive construction in a
rational, discrete-exponent formulation and the complete nonuniform
construction with the real-rate and additive-error quantifiers of
\cref{thm:coefficient}.  \Cref{sec:formal-accounting} records the theorem
endpoints, pinned sources, and finite accounting.
...
\Cref{sec:related} compares prior work before the constructions.
\Cref{sec:two-copy} illustrates disjoint recovery with two requests.
\Cref{sec:code,sec:scheduling,sec:master} develop the code, scheduler, and
composition bound used in the direct proof in \cref{sec:direct};
\cref{sec:high-rate} sharpens the coefficient.
\Cref{sec:quantum} proves the quantum state preparation tradeoff.
An alternative proof in \cref{sec:induction} uses an explicitly constructible
greedy scheduler.  Its quadratic scheduling cost is controlled by grouping
requests and recursively handling resource reuse.
\Cref{app:implementation,sec:formal-accounting} give circuit implementation
details and the exact finite accounting used in Lean.
...
Kretschmer \cite{Kretschmer2023} gives exact synthesis of
...
bound for $t$ uses of an arbitrary $n$-qubit unitary.  His discussion asks
what happens for larger copy ranges.  \Cref{thm:quantum} treats a finite
gate set and constant error for the whole batch; these model differences
are essential to the comparison with exact synthesis.
...
The scheduler maps target coordinates to disjoint recovery sets.  A greedy
construction first shows how the blocking bound gives a circuit of size
$\widetilde O_\ell(g^2q)$ for $g$ requests.  Fixed menus reduce this cost to
$\widetilde O_\ell(gq)$.  Routing then connects the selected addresses to the
resource circuits.
...
We now prove \cref{thm:quantum}.  Its upper bound transfers Boolean mass
production to quantum state preparation.  Its lower bound counts how many
states become distinguishable after taking tensor powers.  Both depend on
charging all elementary gates and measuring error on the entire batch.
...
and CNOT is the controlled-NOT gate.  Every gate has cost one.  A synthesis
circuit starts with all qubits in zero, has $nt$ fixed designated output
qubits, and returns every other qubit exactly to zero.  The number of
scratch qubits is unrestricted.  Circuits have no measurements or
postselection and may depend on the target state.
...
If a Boolean circuit of size $s$ computes $f^{\times t}$, there is a
quantum circuit of size $O(s+t+1)$ implementing
\[
...
\begin{proof}
Compute each Boolean gate into a fresh zero qubit, preserving its source
qubits.  NOT, AND, and OR each require a constant number of NOT, CNOT,
and Toffoli gates; repeated sources and constants are handled by their
one-input or constant simplifications.  These gates have exact
constant-size implementations over $\mathcal G$.  Copy the $t$ output
bits to the $z$ register with CNOTs, and reverse the computation.
Prepared constant sources are also restored.  Repeated use of a source
as a control implements classical fan-out without an additional gate
per use beyond those already charged for computing its successors.
...
The geometric precision allocation is useful even for one copy: the
largest address tables need only $O(L)$ precision bits, while the
extra precision at short tables has a geometrically smaller synthesis
cost.  Related allocations appear in
\cite[Appendix~B]{GossetKothariWu2026}.
...
\cref{thm:quantum}.  The lower bound is a worst-case existence statement;
it does not identify an explicit family of hard states.  Together the
bounds determine the order of growth, not its leading constant, and
do not assert an efficient uniform procedure for finding the circuits.


\section{Implications and open problems}\label{sec:discussion}

The Boolean results leave questions about efficient construction of the
menus, the optimal leading coefficient, and copy rates approaching one.
The quantum application determines the fixed-rate order of growth, but
also leaves a transition near the endpoint.
