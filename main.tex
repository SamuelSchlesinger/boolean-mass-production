...
order-of-growth bound, which is the route covered by the Lean companion.
\end{abstract}
...
An accompanying Lean~4 development formalizes the circuit model, local
recovery, constructive scheduler and routing, composition bound, and
equal-block induction.  It covers the explicit recursive alternative, and
proves a rational, discrete-exponent form of
\cref{thm:main}; the stated real-rate version follows by choosing a slightly
larger rational exponent and absorbing finitely many initial input lengths.
The new nonuniform menu scheduler, its direct proof, and
\cref{thm:coefficient} are not part of this pinned formalization.
\ifanonymous
The public repository URL is omitted during double-blind review.
\else
At revision
\href{https://github.com/SamuelSchlesinger/algebraic-circuits/tree/b94cbedb988f5b4aa72be15de80879333c5e4725}{\texttt{b94cbed}},
the Lean~4.33.1/mathlib~4.33.1 module
\texttt{Algebraic.MassProduction} exposes the endpoint theorem
\texttt{BlockInduction.exponentialMassProduction}; the checked build command
is \texttt{lake build --wfail}.
\fi
...
If at least $\lceil k/2\rceil$ requests are nonclean, a spanning forest of
the nontrivial components has at least $\lceil k/4\rceil$ edges: a component
with $m$ requests and no $0$ contributes $m-1\ge m/2$ edges, and a component
containing $0$ contributes $m$ edges.  Retain exactly
$h=\lceil k/4\rceil$ forest edges.  Root the component containing $0$ there,
and root each other component at any one of its vertices.

For a fixed rooted forest, expose root directions first and then child
directions.  A child is independent of its exposed parent.  Its edge event
has conditional probability at most $a_0$ for parent $0$, and at most $a_1$
for any other parent.  There are $h$ distinct child vertices.  Summing these
product bounds over child sets and parent assignments, including extra
assignments only in the numerical upper bound, gives
\[
  \Pr\{\text{such a forest exists}\}
...
Let $h=\lceil\log_2\ell\rceil$, $q=2^b$, and $N=q^\ell$.  There is a
...
scheduler and the high-rate-code refinement.
