...
% Keep the checked-in manuscript in double-blind submission form.  Switch to
% \anonymousfalse for an identified public or camera-ready version.
\newif\ifanonymous
\anonymousfalse
...
IKOS batch codes \cite{IKOS2004}
  & code-dependent batch size
  & disjoint local recovery; no bound on $\C(f^{\times t})$
  & supplies the closest coding and affine-line antecedent \\
This paper
...
We claim no novelty for evaluation coding, affine-line recovery, or disjoint
recovery as combinatorial ingredients.  The claimed contribution is the
fine-grained deterministic scheduler and fixed-wire scatter/gather circuits,
the bounded-congestion composition that makes their costs affordable, and the
resulting exponential-range mass-production theorem.  A sufficiently sharper
...
\url{https://github.com/SamuelSchlesinger/algebraic-circuits}.
\fi
