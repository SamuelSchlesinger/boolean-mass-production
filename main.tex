...
The depth, randomized-construction, quantum, and lower-bound results
...
\Cref{sec:quantum} proves the quantum state preparation tradeoff, and
\cref{sec:discussion} treats secure evaluation, lower bounds, and open
problems.
...
resource circuits.  The main proof uses the menu scheduler; the greedy
version supplies the explicit recursive alternative in \cref{sec:induction}.
...
The collision estimate below refines the same forest-conditioning argument
used in \cref{lem:partial-schedule}.  To implement a phase at low depth we use
...
\begin{proof}
Form the collision graph on the requests and a vertex $0$ representing
$U$, as in \cref{lem:partial-schedule}.  If more than $v$ requests are
bad, two-color a spanning forest of its nontrivial components.  One color
contains at least $h$ bad requests, not counting vertex $0$.  Choose
exactly $h$ of them as $S$.  Every member of $S$ has a forest neighbor
outside $S$, which may be vertex $0$.
...
$S$ independently chooses a direction whose recovery set meets this
fixed union with probability at most $2gq/D_q$.  A union bound over
the $\binom{k}{h}$ possible choices of $S$ gives the estimate.
\end{proof}
...
Secure evaluation gives a direct application of the simultaneous size
and depth bound.  We then examine what lower bounds say about restricted
circuit architectures and specified hard functions, before returning to
deterministic construction and endpoint copy rates.

\subsection{Secure batch evaluation}\label{sec:secure-evaluation}
The simultaneous size and depth bound has a standard secure-computation
corollary.  Fix integers $m\ge4$ and $0\le c<m/3$, and assume a synchronous
network of $m$ parties with private authenticated channels and broadcast.
For every public $f:\bits^n\to\bits$, fixed $0\le\gamma<1$, and
$1\le t\le2^{\gamma n}$, there is a nonuniform protocol computing
$f^{\times t}$ with perfect security against a static malicious adversary
corrupting at most $c$ parties, using $O_{\gamma,m}(2^n/n)$ communication
bits and $O_{\gamma,m}(n)$ rounds.  The input bits may be distributed
arbitrarily among the parties, and all parties receive the output vector.

To obtain this protocol, apply the Ben-Or--Goldwasser--Wigderson framework
...
\cref{thm:linear-depth}.  Over a fixed field $\F_Q$ of characteristic two
with $Q>m$, AND, OR, and NOT become $xy$, $x+y+xy$, and $1+x$.
First mapping each supplied field element $u$ to $u^{Q-1}$ ensures Boolean
inputs at constant cost per input for fixed $m$.  Secure evaluation then
uses $O_m(S+nt)$ communication bits and $O_m(D+1)$ rounds for a Boolean
circuit of size $S$ and depth $D$, including input sharing and output
reconstruction.  Substitute the simultaneous bounds and
$nt=o_\gamma(2^n/n)$.  The correct circuit and its menus are public
nonuniform protocol data; scheduling and routing are evaluated on shares.

The communication bound comes from secure circuit evaluation.
Circuit lower bounds do not establish its optimality among protocols:
under suitable homomorphic-encryption assumptions, computationally secure
protocols can limit communication according to input and output lengths
and security parameters despite large local computation
\cite[Section~1]{Couteau2018}.  The perfect-security lower bounds in
\cite{DamgaardSchwartzbach2020} concern functions with circuits linear
in their input length and do not settle this larger-circuit regime.
...
\paragraph{A conditional bound for a specified function.}
The counting arguments above do not identify a hard function.  An
inversion assumption gives a bound for the following specified Boolean
family.  A batch can request different bits of one inverse; collecting
those bits recovers the full preimage.

\begin{samepage}
\begin{proposition}[Inverse-bit reduction]\label{prop:inverse-bit}
Let $(P_m)_{m\ge2}$ be a polynomial-time computable family of permutations
of $\bits^m$.  Suppose every nonuniform Boolean circuit $I:\bits^m\to\bits^m$
satisfying
\[
 \Pr_{y\ \mathrm{uniform\ in}\ \bits^m}
       [I(y)=P_m^{-1}(y)]\ge\tfrac12
\]
has at least $H(m)$ gates.  Define a Boolean function on
$m+\lceil\log_2m\rceil$ bits by
\[
 h_m(y,j)=
 \begin{cases}
  (P_m^{-1}(y))_j,&j\in[m]_0,\\
  0,&\text{otherwise}.
 \end{cases}
\]
...
\begin{proof}
Let a circuit of size $S$ compute $h_m^{\times t}$ exactly.
Use $\lceil m/t\rceil$ copies of it, wiring the same $y$ into every
request slot and hardwiring the indices $0,\ldots,m-1$ across those
slots.  Pad any remaining slots with index zero and discard their
outputs.  The retained outputs give $P_m^{-1}(y)$ for every $y$,
using at most $\lceil m/t\rceil S$ gates.  The assumption gives the
inequality.
\end{proof}

For $1\le t\le m$, the lower bound is at least $tH(m)/(2m)$; for
$t\ge m$, it is $H(m)$.  Exponential inversion hardness therefore gives
an exponential circuit lower bound, but this reduction supplies no
further growth once $t\ge m$.  The predicate $h_m$ itself need not be
efficiently evaluable.  The result neither compares $\C(h_m^{\times t})$
with $t\C(h_m)$ nor closes the unrestricted leading-coefficient gap.
...
\bibitem[Cou18]{Couteau2018}
Geoffroy Couteau.
\newblock A note on the communication complexity of multiparty computation
in the correlated randomness model.
\newblock Cryptology ePrint Archive, Report 2018/465, 2018.
\newblock \url{https://eprint.iacr.org/2018/465}.

\bibitem[DS20]{DamgaardSchwartzbach2020}
Ivan Damg{\aa}rd and Nikolaj I. Schwartzbach.
\newblock Communication lower bounds for perfect maliciously secure MPC.
\newblock Cryptology ePrint Archive, Report 2020/251, 2020.
\newblock \url{https://eprint.iacr.org/2020/251}.
