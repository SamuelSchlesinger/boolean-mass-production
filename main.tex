...
The scheduler has size $\widetilde O_\ell(tq)$, where $q$ is the field size and
$\ell$ is the fixed geometric dimension.  It tests fixed menus of
...
efficient uniform construction is given.  The circuit bound includes schedule
selection, resource evaluation, routing, and decoding.  An explicit greedy
scheduler also gives a recursive proof of the order-of-growth bound.
A Lean companion formally verifies both upper-bound
...
the sharp one-copy coefficient is $1$ \cite{FrandsenMiltersen2005}.
...
Global truth-table lookup also handles exponentially many requests:
Paul's bound is $O(n2^n)$ in our
...
Our scheduler selects recovery sets from the input addresses within a gate
budget small enough to obtain the stated circuit bound.  \Cref{sec:related}
gives the detailed comparison and attribution.

\paragraph{Machine-checked companion.}
An accompanying Lean~4 development verifies the recursive construction in a
...
Here ``one-copy leading term'' means the basis-dependent Shannon--Lupanov
worst-case asymptotic.  Uhlig's 1974 paper \cite{Uhlig1974} is a precursor to
...
The results leave three questions: efficient construction of the menus, the
optimal leading coefficient, and the behavior at copy rates approaching one.
...
through the sharp real-rate theorem.
